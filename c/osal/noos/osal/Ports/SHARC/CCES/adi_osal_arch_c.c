/* $Revision: 16164 $
 * $Date: 2012-09-04 16:10:03 +0800 (Tue, 04 Sep 2012) $
******************************************************************************
...
/*!
  ****************************************************************************
    @brief  Initialises the high-level interrupt handling.
...
*****************************************************************************/
ADI_OSAL_STATUS _adi_osal_InitInterrupts(void)
{

    /* On blackfin processors we need to dynamically allocate and modify the
     * osal handler table.  On SHARC there are a fixed number of interrupts,
     * and so a fixed number of handlers.  Therefore the OSAL handler table
     * can be statically allocated so we just return success. */
    return ADI_OSAL_SUCCESS;
}
...
)
{
    ...
    /* Call the low-level dispatcher to install the OSAL handler */
    index = adi_rtl_register_dispatched_handler (iid,
                                                 _adi_osal_stdWrapper,
                                                 (adi_dispatched_callback_t) handlerArg);
    if (index != -1)
    {
...
adi_osal_ActivateHandler (uint32_t iid)
{
    ...
    if (-1 != adi_rtl_activate_dispatched_handler(iid))
    {
        ret = ADI_OSAL_SUCCESS;
    }
...
adi_osal_DeactivateHandler (uint32_t iid)
{
    ...
    if (-1 != adi_rtl_deactivate_dispatched_handler(iid))
    {
        ret = ADI_OSAL_SUCCESS;
    }
...
adi_osal_UninstallHandler (uint32_t iid)
{
    ...
    if (index != -1)
    {
        /* Handler unregistered correctly */
